...
function starPositions(arms, rOuter, rInner) {
    ...
    var pos = [];
    for (var i_1 = 0; i_1 < 2 * arms; i_1++) {
        var r = (i_1 % 2) === 0 ? rOuter : rInner;
        var p = new Cesium.Cartesian2(Math.cos(i_1 * angle) * r, Math.sin(i_1 * angle) * r);
        pos.push(p);
    }
    return pos;
}
...
function computeCircle(radius) {
    var positions = [];
    for (var i_2 = 0; i_2 < 360; i_2++) {
        var radians = Cesium.Math.toRadians(i_2);
